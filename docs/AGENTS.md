...
* **FinOps & Costes**: `bq-dry-run-optimizer` + `multi_tenancy_security_specialist`.

### C. `AppViajes` (Flutter / Movilidad H3 / OSRM)
* **Algoritmos & Rendimiento**: `python-vectorization-optimizer` + `h3-surge-calculator` + `queue-bottleneck-detector`.
