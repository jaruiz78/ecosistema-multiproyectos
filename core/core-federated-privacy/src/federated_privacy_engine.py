...
class FederatedPrivacyEngine:
    ...
    def apply_laplace_noise(self, value: float) -> float:
        import numpy as np
        # Laplace mechanism: noise ~ Lap(epsilon / sensitivity)
        # Using inverse transform sampling for Laplace
        u = np.random.uniform(-0.5, 0.5)
        b = self.sensitivity / self.epsilon
        noise = -b * np.sign(u) * np.log(1 - 2 * abs(u))
        return value + noise

    def privatize_telemetry_metric(self, raw_value: float, sensitivity: float = 1.0) -> float:
        import numpy as np
        scale = sensitivity / self.epsilon
        # Real mathematical Laplace noise with O(1) computation
        u = np.random.uniform(-0.5, 0.5)
